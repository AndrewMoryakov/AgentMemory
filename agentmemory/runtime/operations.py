...
OPERATIONS: dict[str, OperationSpec] = {
    ...
    "add": OperationSpec(
        ...
        description=(
            ...
            "is observable. With infer=true the provider may also split the input into "
            "several distinct facts and store each separately; the primary record returned "
            "represents the first such write, and the remainder are surfaced under "
            "additional_records — every entry is a full memory record with its own id. "
            "infer=true costs one LLM call per write. Optional lifecycle: set "
            "metadata.ttl_seconds (positive number) or metadata.expires_at (ISO 8601 UTC) "
