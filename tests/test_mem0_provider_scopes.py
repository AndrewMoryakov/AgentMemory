...
class Mem0ProviderScopeInventoryTests(unittest.TestCase):
    ...
    def _seed_registry(self, records: list[dict[str, object]]) -> None:
        scope_registry.replace_provider_records(
            self.provider.provider_name,
            [
                self.provider._normalize_record({"id": f"seed-{index}", **record})
    ...
    def test_list_scopes_uses_registry_not_legacy_storage_reader(self) -> None:
        self._seed_registry([{"user_id": "default", "updated_at": "2026-04-09T10:00:00+00:00"}])
        self.provider._iter_scope_payloads = lambda: (_ for _ in ()).throw(AssertionError("legacy reader should not be used"))  # type: ignore[method-assign]
